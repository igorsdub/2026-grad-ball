---
title: "OIST Graduation Ball 2026"
---


<big>The biggest event of the year! Join us for the  annual celebration of the OIST graduates.</big>


```{=html}
<div class="countdown-container">
  <div class="countdown-title">Days Until the Ball</div>
  <div class="countdown-number" id="countdown">—</div>
</div>

<script>
  function updateCountdown() {
    const eventDate = new Date('2026-02-27T00:00:00').getTime();
    const today = new Date().getTime();
    const daysRemaining = Math.ceil((eventDate - today) / (1000 * 60 * 60 * 24));
    const maxDays = 60; // Starting point for blue color
    
    document.getElementById('countdown').textContent = Math.max(0, daysRemaining);
    
    // Calculate color transition from blue to red based on days remaining
    let color1, color2;
    
    if (daysRemaining > maxDays) {
      // All blue
      color1 = '#667eea';
      color2 = '#764ba2';
    } else if (daysRemaining < 0) {
      // All red
      color1 = '#f093fb';
      color2 = '#f5576c';
    } else {
      // Transition from blue to red
      const ratio = 1 - (daysRemaining / 19);
      
      // Blue: #667eea → Red: #f093fb
      const r1 = Math.round(102 + (240 - 102) * ratio);
      const g1 = Math.round(126 + (147 - 126) * ratio);
      const b1 = Math.round(234 + (251 - 234) * ratio);
      color1 = `rgb(${r1}, ${g1}, ${b1})`;
      
      // Blue: #764ba2 → Red: #f5576c
      const r2 = Math.round(118 + (245 - 118) * ratio);
      const g2 = Math.round(75 + (87 - 75) * ratio);
      const b2 = Math.round(162 + (108 - 162) * ratio);
      color2 = `rgb(${r2}, ${g2}, ${b2})`;
    }
    
    const countdownElement = document.querySelector('.countdown-container');
    countdownElement.style.background = `linear-gradient(135deg, ${color1} 0%, ${color2} 100%)`;
  }
  
  updateCountdown();
</script>

<div class="progress-container">
  <div class="progress-title">Tickets Sold</div>
  <div class="progress-bar-wrapper">
    <div class="progress-bar">
      <div class="progress-fill progress-striped progress-animated" id="progressFill"></div>
    </div>
  </div>
</div>

<script>
  // Update ticket progress bar
  // Edit ticketsSold and totalTickets values below
  const ticketsSold = 147;
  const totalTickets = 200;
  
  function updateProgressBar() {
    const percentage = (ticketsSold / totalTickets) * 100;
    document.getElementById('progressFill').style.width = percentage + '%';
    document.getElementById('progressText').textContent = ticketsSold + ' / ' + totalTickets;
  }
  
  updateProgressBar();
</script>

<div class="raffle-container">
  <div class="raffle-title">🎰 Raffle</div>
  <p style="margin: 1rem 0 1.5rem 0; font-size: 1rem; text-align: center;">Buy your ticket by <strong>February 20th</strong> to enter our raffle and win:</p>
  <ol style="margin: 1rem auto; padding-left: 2rem; font-size: 1rem; max-width: 500px; text-align: left;">
    <li style="margin: 0.8rem 0;">🏨 Free stay voucher at Miyuki Beach Hotel <a href="https://www.instagram.com/hotelmiyukibeach_official"><img src="images/instagram-icon-white.svg" class="instagram-img" style="width: 16px; height: 16px; vertical-align: middle; margin-left: 0.5rem;"></a></li>
    <li style="margin: 0.8rem 0;">🏨 Free stay voucher at Rizzan Sea-Park Hotel</li>
    <li style="margin: 0.8rem 0;">🍽️ Dinner for two voucher at Rizzan</li>
    <li style="margin: 0.8rem 0;">🍽️ Lunch for two voucher at Rizzan</li>
  </ol>
</div>
```

```{=html}
<div class="three-column-layout">
```


::: section-container

## 📋 Event Details
🗓️ 2026 Feb 27<sup>th</sup> Fri  
🕗 20:00 - late (doors open at 19:30)  
📍 OIST Center Building 

🤵 Cocktail Attire or National Dress  
🎟️ ¥5,000 (food, drinks, and more)
:::

::: section-container

## 🎫 Ticket Sales

Every weekday during lunch time (12:00-13:00) at Cafe Tancha big table.  
**<span style="color: orange;">Number of tickets is limited!</span>**


Can't make it at lunchtime? Email us at [graduationball@oist.jp](mailto:graduationball@oist.jp).

:::

::: section-container-featured

## 🎉 Entertainment

🎸 Andrew and Fr. <a href="https://www.instagram.com/andrew.and.fr"><img src="images/instagram-icon-white.svg" class="instagram-img" style="width: 20px; height: 20px; vertical-align: middle; margin-right: 0.5rem;"></a>   
🎷 Modern Jazz Orchestra (MOJO) <a href="https://www.instagram.com/ryudaimojo"><img src="images/instagram-icon-white.svg" class="instagram-img" style="width: 20px; height: 20px; vertical-align: middle; margin-right: 0.5rem;"></a>   
🎧 DJ Skeezy <a href="https://www.instagram.com/djskeezy"><img src="images/instagram-icon-white.svg" class="instagram-img" style="width: 20px; height: 20px; vertical-align: middle; margin-right: 0.5rem;"></a>  
🎤 Karaoke room  
📸 Photo booth
:::

::: section-container

## 🍾 Catering

🍹 All-you-can-drink by Lab0  
🍽️ Finger foods by Caesar's Kitchen

Detailed menu to be announced soon.
:::

```{=html}
</div>
```


## OIST Graduation Ball 2019
This year will be the first time the Graduation Ball will be held on campus since 2019.

```{=html}
<div class="gallery-container">
  <div class="gallery-wrapper">
    <img class="gallery-image" src="images/2019-oist-grad-ball_01.jpg" alt="2019 Graduation Ball - Photo 1">
    <img class="gallery-image" src="images/2019-oist-grad-ball_02.jpg" alt="2019 Graduation Ball - Photo 2">
    <img class="gallery-image" src="images/2019-oist-grad-ball_03.jpg" alt="2019 Graduation Ball - Photo 3">
    <img class="gallery-image" src="images/2019-oist-grad-ball_05.jpg" alt="2019 Graduation Ball - Photo 4">
    <img class="gallery-image" src="images/2019-oist-grad-ball_06.jpg" alt="2019 Graduation Ball - Photo 5">
</div>
  <button class="gallery-nav gallery-prev" onclick="changeGalleryImage(-1)">❮</button>
  <button class="gallery-nav gallery-next" onclick="changeGalleryImage(1)">❯</button>
  <div class="gallery-dots">
    <span class="dot active" onclick="currentGalleryImage(0)"></span>
    <span class="dot" onclick="currentGalleryImage(1)"></span>
    <span class="dot" onclick="currentGalleryImage(2)"></span>
    <span class="dot" onclick="currentGalleryImage(3)"></span>
    <span class="dot" onclick="currentGalleryImage(4)"></span>
  </div>
</div>

<script>
let galleryIndex = 0;
let galleryTimer;

function changeGalleryImage(n) {
  clearTimeout(galleryTimer);
  galleryIndex += n;
  if (galleryIndex >= document.querySelectorAll('.gallery-image').length) {
    galleryIndex = 0;
  }
  if (galleryIndex < 0) {
    galleryIndex = document.querySelectorAll('.gallery-image').length - 1;
  }
  displayGalleryImage();
  autoGallery();
}

function currentGalleryImage(n) {
  clearTimeout(galleryTimer);
  galleryIndex = n;
  displayGalleryImage();
  autoGallery();
}

function displayGalleryImage() {
  const images = document.querySelectorAll('.gallery-image');
  const dots = document.querySelectorAll('.dot');
  
  images.forEach(img => img.style.display = 'none');
  images[galleryIndex].style.display = 'block';
  
  dots.forEach(dot => dot.classList.remove('active'));
  dots[galleryIndex].classList.add('active');
}

function autoGallery() {
  galleryTimer = setTimeout(() => {
    galleryIndex++;
    if (galleryIndex >= document.querySelectorAll('.gallery-image').length) {
      galleryIndex = 0;
    }
    displayGalleryImage();
    autoGallery();
  }, 4000);
}

// Initialize gallery
displayGalleryImage();
autoGallery();
</script>
```


::: section-container-featured

## ⚠️ Safe Space Policy

All participants are expected to contribute to a **safe and respectful environment**. This event is a safe space for women and the LGBTQIA+ community. 

**We have a zero tolerance policy towards harassment of any kind** on the basis of gender, sexuality, race, outfit choice, or any other characteristic.

Harassment, discrimination, or offensive behavior are not tolerated. Individuals who violate these expectations may be asked to leave and will be subjected to OIST anti-harassment policies.

:::

## ℹ️ FAQs

**Where exactly is the event?**  
Different locations within the OIST Center Building. The main event will be held in Caesar's Kitchen.

**I am graduating. Do I need to get a ticket?**  
No, graduates get free entry. Please email us at [graduationball@oist.jp](mailto:graduationball@oist.jp).

**How do I check in on the day of the event?** 
If you purchased your ticket, you should’ve received an individual QR code that will be scanned on the day of the event. If you haven’t received it yet, contact [graduationball@oist.jp](mailto:graduationball@oist.jp).

**Guest parking?**  
OIST's main parking building is available for use.

**Can I bring a guest?**  
Yes, but they will need to purchase a ticket for them.


::: section-container-sponsors
## 🙏 Sponsors
<img src="images/cezars-kitchen-logo-white.svg" alt="Caesar's Kitchen Logo" class="sponsor-logo" style="width: auto; height: auto; margin-right: 2rem;">

:::

::: section-container
## 💝 Donors

We would like to thank our OIST community. Every donation, big or small, helps us make this night unforgettable for our graduates.

Amedeo Roberto Esposito, Amy Shen, Daniel Mueller, Dyon van Dinter, Gaspar Sanchez, Hideaki Kawai, Hugo Hoedemaker, Isaku Higa, Jamie Sanderson, Jamila Rodrigues, Julio Barros, Kacper Nowak, Kenji Doya, Lilian Magnus, Okay Cem, Pinaki Chakraborty, Samuel Ross, Satoshi Mitarai, Shigeharu Kato, Shinji Narita, Simone Pigolotti, Tom Froese, Yifan Wang, Yukiko Gouda

:::

::: section-container-contact
## 📬 Contact Us
Find us at Cafe Tancha during ticket sales or email us at [graduationball@oist.jp](mailto:graduationball@oist.jp).
:::

::: section-container-organizers
## 👩‍💼 Organizers
Alessandra De Amici, Anzhelika Koldaeva, Christian Amor Rodriguez, Diala Edde, Ian Christopher Johnson, Jann Zwahlen, Kokila Dilhani Perera, Lorena Andreoli, Maya Agata, Maya Street, Miyu Nambu, Simone Tandurella, and Xim Yue Hong.
:::